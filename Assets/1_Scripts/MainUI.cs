...
public class MainUI : MonoBehaviour
{
    ...
    private float maxHP = 100; // 플레이어 최대 hp
    private float curHP = 100; // 플레이어 현재 hp
    //public Human hp;
    private bool ispause = false; //menuSet 호출시 true
    int cur_Bullet = 10; // 플레이어의 공격횟수를 받아 올 것 (현재는 임의 값 10을 준 상태)
    ...
    public void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space)) // 공격 당할 때
        {
            curHP -= 10; 
        }
        HandleHP();    
    ...
    public void Show_Bullet_Count()  // 현재 총알 수 함수
    {
        ...
        if(Input.GetKeyDown(KeyCode.R) || cur_Bullet <= 0)
        {
            cur_Bullet = 10;
            bullet_Count.text = cur_Bullet.ToString();
        }
